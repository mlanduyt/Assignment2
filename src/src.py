...
def elastic_critical_load_analysis(elements, displacements):
    ...
    for element in elements:
        ...
        Iy = element.Iy
        J = element.J
        k_g = local_geometric_stiffness_matrix_3D_beam(L, A, Ip, *displacements[:6])
        k_e = local_elastic_stiffness_matrix_3D_beam(E, nu, A, L, Iy, Iz, J)
        eigvalues, eigvectors = scipy.linalg.eig(k_e, -k_g)
        critical_loads.append(min(eigvalues))
        return eigvalues, eigvectors, np.array(critical_loads)
